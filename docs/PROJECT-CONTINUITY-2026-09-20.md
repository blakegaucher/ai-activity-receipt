...
## 20. Repository integrity / continuity consolidation — 2026-09-20

A source-first review of current `main` confirms that the repository has now added two broad repository-integrity guards on top of the existing specialized validators:

- deterministic repository-local Markdown link validation;
- repository-wide UTF-8 JSON parsing with duplicate-key rejection and draft-2020-12 schema meta-validation.
...
```text
project-continuity-v0.10
snapshot_date = 2026-09-20
```

This update does **not** change any experimental, commercial, licensing, security-admin, or human-study evidence state.

### Current unattended-work boundary

The current roadmap has no remaining high-value engineering task that should be completed automatically merely to create more repository activity.

The open gates now require one or more of:

- explicit owner/IP choice (repository license);
- repository-admin action (main protection, private vulnerability reporting, Dependabot/security-alert settings, CodeQL alert inspection);
- real browser/device interaction;
- deliberate human-study methodology decisions;
- ethics/REB/IRB review or determination as applicable;
- realistic external traces/evidence;
- independent external reproduction;
- production identity/key/revocation infrastructure.

Do not claim these gates are complete because the repository-local test suite is green.
...
The frozen AR-P003 v0.2.3 historical baseline remains append-only.

The public canonical record remains `candidate-record-v0.1`; multi-hop v0.2 remains a research profile.

The repository remains without an explicit open-source license until Blake Gaucher / Ancient Immortal Art makes that decision.
