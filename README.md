...
The v0.3 protocol is **not frozen or executed**. Five methodology choices are now explicit: the three-condition structured-control comparison, integrated challenge strata, a relevant-professional reviewer population with prespecified eligibility/exclusion and no-external-assistance rules, the binary primary endpoint `correct_completion_by_180s`, and `primary_timing_clock = wall_deadline_with_hidden_sensitivity`. Wall elapsed time is primary; hidden/background time remains inside the 180-second deadline; active time remains secondary/sensitivity. The meaningful effect/precision target remains unresolved, and the timing/exclusions implementation gate still requires timeout/technical-failure/browser-device freeze checks. Reviewer recruitment is not authorized. Historical alternatives and source conflicts remain preserved in the [Methodology Decision Ledger](docs/AR-P003-V0.3-METHODOLOGY-DECISIONS.md), [Primary Endpoint Decision](docs/AR-P003-V0.3-PRIMARY-ENDPOINT-DECISION-2026-09-21.md), [Primary Timing Clock Decision](docs/AR-P003-V0.3-PRIMARY-TIMING-CLOCK-DECISION-2026-09-23.md), and [Decision Support](docs/AR-P003-V0.3-DECISION-SUPPORT.md).
