...
The v0.3 protocol is **not frozen or executed**. Four methodology choices are now explicit: the three-condition structured-control comparison, integrated challenge strata, a relevant-professional reviewer population with prespecified eligibility/exclusion and no-external-assistance rules, and the binary primary endpoint `correct_completion_by_180s`. The endpoint keeps component outcomes secondary and critical false clearance separate. Active versus wall/deadline primary timing semantics and the meaningful effect/precision target remain unresolved, so confirmatory endpoint derivation remains fail-closed and reviewer recruitment is not authorized. Historical alternatives and source conflicts remain preserved in the [Methodology Decision Ledger](docs/AR-P003-V0.3-METHODOLOGY-DECISIONS.md), [Primary Endpoint Decision](docs/AR-P003-V0.3-PRIMARY-ENDPOINT-DECISION-2026-09-21.md), and [Decision Support](docs/AR-P003-V0.3-DECISION-SUPPORT.md).
