...
GitHub API inspection on 2026-09-19 returned no repository rulesets. CodeQL **advanced setup** is now committed and has completed successful Python and JavaScript/TypeScript analyses on `main`. Main-branch ruleset/protection, private vulnerability reporting, confirmation of Dependabot security alerts/security updates, owner security-alert notifications, and manual inspection of the CodeQL alert page remain repository-admin tasks and must not be described as completed until verified.
